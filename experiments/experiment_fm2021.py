...
logging.basicConfig(
    filename=f"temp-files/log/{str(datetime.time(datetime.now()))}_experiment_fm2021_smc.log",
    level=logging.DEBUG,
)
import numpy as np
...
class ExperimentFm2021Config(ExperimentConfig):
    _config = {
        "zeroconf_10_fulldata": {
            ...
            "interval": (0, 1),
            "true_param": np.array([0.39980148, 0.69081848]),
            "observed_data": np.array([162, 9838]),
            "observed_labels": ["err", "ok"],
        },
        "zeroconf_10_few_data": {
            "smc_trace_len": 200,
            "smc_pertubation_len": 20,
            "smc_mh_trace_len": 50,
            "abc_threshold": 0.25,
        ...
        "jchain_10_full_data": {
            ...
            "smc_mh_trace_len": 50,
            "abc_threshold": 0.25,
            "prism_model_file": "/home/huypn12/Works/mcss/bbeess-py/data/prism/jchain.pm",
            "prism_props_file": "/home/huypn12/Works/mcss/bbeess-py/data/prism/jchain.pctl",
            "interval": (0, 1),
            "true_param": np.array(
                [0.34, 0.25, 0.2, 0.4, 0.21, 0.64, 0.26, 0.77, 0.34, 0.65]
            ),
            "observed_data": np.array([320, 182, 112, 144, 51, 120, 17, 37, 6, 6, 5]),
            "observed_labels": [
                "bscc_11",
                "bscc_12",
                "bscc_13",
        ...
        "jchain_10_few_data": {
            ...
            "observed_data": np.array([156, 3, 32, 5, 1, 1, 0, 0, 1, 1]),
            "observed_labels": [
                "bscc_11",
                "bscc_12",
                "bscc_13",
        ...
        "jchain_2_full_data": {
            ...
            "smc_mh_trace_len": 50,
            "abc_threshold": 0.25,
            "prism_model_file": "/home/huypn12/Works/mcss/bbeess-py/data/prism/jchain2.pm",
            "prism_props_file": "/home/huypn12/Works/mcss/bbeess-py/data/prism/jchain2.pctl",
            "interval": (0, 1),
            "true_param": np.array([0.34, 0.25]),
            "observed_data": np.array([160, 74, 266]),
            "observed_labels": ["bscc_1", "bscc_2", "bscc_3"],
        },
        "jchain_2_few_data": {
            "smc_trace_len": 200,
            "smc_pertubation_len": 20,
...
def main(cfg_name: str, mode: EvaluationMode):
    ...
    experiment.run()
    sim = experiment.model.simulate(np.array([0.34, 0.25]), 1000000)
    print(sim * 1.0 / sum(sim))


def main_all():
    for eval_mode in [EvaluationMode.RationalFunction, EvaluationMode.Simulation]:
...
def manual():
    for eval_mode in [EvaluationMode.Simulation]:
        for cfg in ["jchain_2_full_data", "jchain_10_full_data"]:
            main(cfg, eval_mode)
